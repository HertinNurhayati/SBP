...
# Step 2: Calculate average solution (AV)
def calculate_av(matriks):
    return np.mean(matriks, axis=0)

# Step 3: Calculate positive (PDA) and negative (NDA) distances from the average
...
# Step 4: Calculate weighted sums of PDA and NDA (SP, SN)
def calculate_sp_sn(PDA, NDA, bobot):
    SP = np.sum(PDA * bobot, axis=1)
    SN = np.sum(NDA * bobot, axis=1)
    return SP, SN

# Step 5: Normalize the values of SP and SN (NSP, NSN)
